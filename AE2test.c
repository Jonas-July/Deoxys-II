/*
	Copyright 2022 Jonas July.
	Licensed under the EUPL-1.2-or-later.
	You may obtain a copy of the Licence at:
	https://joinup.ec.europa.eu/collection/eupl

	Test cases for the Deoxys-AE2 cipher

	Last modified: 2022-07-23 by Jonas July
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#include "deoxys_AE2.h"

// Test vector 1
int test_case_no_message_no_ad() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {};
	uint8_t ad[] = {};
	uint8_t expected[] = {0x2b, 0x97, 0xbd, 0x77, 0x71, 0x2f, 0x0c, 0xde, 0x97, 0x53, 0x09, 0x95, 0x9d, 0xfe, 0x1d, 0x7c};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 0, ad, 0);
	int failed = memcmp(crypt, expected, 16);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 2
int test_case_no_message_with_ad() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint8_t expected[] = {0x54, 0x70, 0x8a, 0xe5, 0x56, 0x5a, 0x71, 0xf1, 0x47, 0xbd, 0xb9, 0x4d, 0x7b, 0xa3, 0xae, 0xd7};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 0, ad, 32);
	int failed = memcmp(crypt, expected, 16);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 3
int test_case_no_message_with_ad2() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {};
	uint8_t ad[] = {0xf4, 0x95, 0xc9, 0xc0, 0x3d, 0x29, 0x98, 0x96, 0x95, 0xd9, 0x8f, 0xf5, 0xd4, 0x30, 0x65, 0x01, 0x25, 0x80, 0x5c, 0x1e, 0x05, 0x76, 0xd0, 0x6f, 0x26, 0xcb, 0xda, 0x42, 0xb1, 0xf8, 0x22, 0x38, 0xb8};
	uint8_t expected[] = {0x32, 0x77, 0x68, 0x9d, 0xc4, 0x20, 0x8c, 0xc1, 0xff, 0x59, 0xd1, 0x54, 0x34, 0xa1, 0xba, 0xf1};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 0, ad, 33);
	int failed = memcmp(crypt, expected, 16);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 4
int test_case_with_message_no_ad() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint8_t ad[] = {};
	uint8_t expected[] = {0x9d, 0xa2, 0x0d, 0xb1, 0xc2, 0x78, 0x1f, 0x66, 0x69, 0x25, 0x7d, 0x87, 0xe2, 0xa4, 0xd9, 0xbe, 0x19, 0x70, 0xf7, 0x58, 0x1b, 0xef, 0x2c, 0x99, 0x5e, 0x11, 0x49, 0x33, 0x1e, 0x5e, 0x8c, 0xc1, 0x92, 0xce, 0x3a, 0xec, 0x3a, 0x4b, 0x72, 0xff, 0x9e, 0xab, 0x71, 0xc2, 0xa9, 0x34, 0x92, 0xfa};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 32, ad, 0);
	int failed = memcmp(crypt, expected, 48);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 5
int test_case_with_message_no_ad2() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {0x15, 0xcd, 0x77, 0x73, 0x2f, 0x9d, 0x0c, 0x4c, 0x6e, 0x58, 0x1e, 0xf4, 0x00, 0x87, 0x6a, 0xd9, 0x18, 0x8c, 0x5b, 0x88, 0x50, 0xeb, 0xd3, 0x82, 0x24, 0xda, 0x95, 0xd7, 0xcd, 0xc9, 0x9f, 0x7a, 0xcc};
	uint8_t ad[] = {};
	uint8_t expected[] = {0xe5, 0xff, 0xd2, 0xab, 0xc5, 0xb4, 0x59, 0xa7, 0x36, 0x67, 0x75, 0x6e, 0xda, 0x64, 0x43, 0xed, 0xe8, 0x6c, 0x08, 0x83, 0xfc, 0x51, 0xdd, 0x75, 0xd2, 0x2b, 0xb1, 0x49, 0x92, 0xc6, 0x84, 0x61, 0x8c, 0x5f, 0xa7, 0x8d, 0x57, 0x30, 0x8f, 0x19, 0xd0, 0x25, 0x20, 0x72, 0xee, 0x39, 0xdf, 0x5e, 0xcc};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 33, ad, 0);
	int failed = memcmp(crypt, expected, 49);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 6
int test_case_with_message_with_ad() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
	uint8_t expected[] = {0x10, 0x9f, 0x8a, 0x16, 0x8b, 0x36, 0xdf, 0xad, 0xe0, 0x26, 0x28, 0xa9, 0xe1, 0x29, 0xd5, 0x25, 0x7f, 0x03, 0xcc, 0x79, 0x12, 0xae, 0xfa, 0x79, 0x72, 0x9b, 0x67, 0xb1, 0x86, 0xa2, 0xb0, 0x8f, 0x65, 0x49, 0xf9, 0xbf, 0x10, 0xac, 0xba, 0x0a, 0x45, 0x1d, 0xbb, 0x24, 0x84, 0xa6, 0x0d, 0x90};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 32, ad, 16);
	int failed = memcmp(crypt, expected, 48);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 7
int test_case_with_message_with_ad2() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {0x42, 0x28, 0x57, 0xfb, 0x16, 0x5a, 0xf0, 0xa3, 0x5c, 0x03, 0x19, 0x9f, 0xb8, 0x95, 0x60, 0x4d, 0xca, 0x9c, 0xea, 0x6d, 0x78, 0x89, 0x54, 0x96, 0x2c, 0x41, 0x9e, 0x0d, 0x5c, 0x22, 0x5c, 0x03, 0x27};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10};
	uint8_t expected[] = {0x7d, 0x77, 0x22, 0x03, 0xfa, 0x38, 0xbe, 0x29, 0x6d, 0x8d, 0x20, 0xd8, 0x05, 0x16, 0x31, 0x30, 0xc6, 0x9a, 0xba, 0x8c, 0xb1, 0x6e, 0xd8, 0x45, 0xc2, 0x29, 0x6c, 0x61, 0xa8, 0xf3, 0x4b, 0x39, 0x4e, 0x0b, 0x3f, 0x10, 0xe3, 0x93, 0x3c, 0x78, 0x19, 0x0b, 0x24, 0xb3, 0x30, 0x08, 0xbf, 0x80, 0xe9};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 33, ad, 17);
	int failed = memcmp(crypt, expected, 49);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 8
int test_case_extreme() {
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t message[] = {};
	uint8_t ad[] = {};
	uint8_t expected[] = {};

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 512, ad, 513);
	int failed = memcmp(crypt, expected, 528);
	free(crypt);

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}


// Performance test
int performance_test_1MB() {
	clock_t begin = clock();

	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t* message = calloc(1048576, sizeof(uint8_t));
	uint8_t* ad = calloc(1048576, sizeof(uint8_t));

	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 1048576, ad, 1048576);

	clock_t end = clock();
	free(crypt);
	free(message);
	free(ad);

	double taken = (double)(end - begin) / CLOCKS_PER_SEC;
	printf("Time taken for encrypting 1MB: %fs\n", taken);
	return taken;
}

// Performance test
int performance_test_1GB() {

	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t* message = calloc(1073741824, sizeof(uint8_t));
	uint8_t* ad = calloc(1073741824, sizeof(uint8_t));

	clock_t begin = clock();
	uint8_t* crypt = Deoxys_AE2_encrypt_buffer(key, nonce, message, 1073741824, ad, 1073741824);
	clock_t end = clock();

	free(crypt);
	free(message);
	free(ad);

	double taken = (double)(end - begin) / CLOCKS_PER_SEC;
	printf("Time taken for encrypting 1GB: %fs\n", taken);
	return taken;
}


// Test vector 1
int test_case_dec_no_message_no_ad()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x2b, 0x97, 0xbd, 0x77, 0x71, 0x2f, 0x0c, 0xde, 0x97, 0x53, 0x09, 0x95, 0x9d, 0xfe, 0x1d, 0x7c};
	uint8_t ciphertext[] = {};
	uint8_t ad[] = {};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 1
int test_case_dec_no_message_no_ad_wrong_tag()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x00, 0x97, 0xbd, 0x77, 0x71, 0x2f, 0x0c, 0xde, 0x97, 0x53, 0x09, 0x95, 0x9d, 0xfe, 0x1d, 0x7c};
	uint8_t ciphertext[] = {};
	uint8_t ad[] = {};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 1;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 2
int test_case_dec_no_message_with_ad()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x54, 0x70, 0x8a, 0xe5, 0x56, 0x5a, 0x71, 0xf1, 0x47, 0xbd, 0xb9, 0x4d, 0x7b, 0xa3, 0xae, 0xd7};
	uint8_t ciphertext[] = {};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 32);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 2
int test_case_dec_no_message_with_ad_wrong_tag()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x00, 0x00, 0x8a, 0xe5, 0x56, 0x5a, 0x71, 0xf1, 0x47, 0xbd, 0xb9, 0x4d, 0x7b, 0xa3, 0xae, 0xd7};
	uint8_t ciphertext[] = {};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 1;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 32);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 3
int test_case_dec_no_message_with_ad2()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x54, 0x70, 0x8a, 0xe5, 0x56, 0x5a, 0x71, 0xf1, 0x47, 0xbd, 0xb9, 0x4d, 0x7b, 0xa3, 0xae, 0xd7};
	uint8_t ciphertext[] = {};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 32);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 4
int test_case_dec_with_message_no_ad()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x92, 0xce, 0x3a, 0xec, 0x3a, 0x4b, 0x72, 0xff, 0x9e, 0xab, 0x71, 0xc2, 0xa9, 0x34, 0x92, 0xfa};
	uint8_t ciphertext[] = {0x9d, 0xa2, 0x0d, 0xb1, 0xc2, 0x78, 0x1f, 0x66, 0x69, 0x25, 0x7d, 0x87, 0xe2, 0xa4, 0xd9, 0xbe, 0x19, 0x70, 0xf7, 0x58, 0x1b, 0xef, 0x2c, 0x99, 0x5e, 0x11, 0x49, 0x33, 0x1e, 0x5e, 0x8c, 0xc1};
	uint8_t ad[] = {};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint32_t expected_size = 32;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 32, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 5
int test_case_dec_with_message_no_ad2()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x5f, 0xa7, 0x8d, 0x57, 0x30, 0x8f, 0x19, 0xd0, 0x25, 0x20, 0x72, 0xee, 0x39, 0xdf, 0x5e, 0xcc};
	uint8_t ciphertext[] = {0xe5, 0xff, 0xd2, 0xab, 0xc5, 0xb4, 0x59, 0xa7, 0x36, 0x67, 0x75, 0x6e, 0xda, 0x64, 0x43, 0xed, 0xe8, 0x6c, 0x08, 0x83, 0xfc, 0x51, 0xdd, 0x75, 0xd2, 0x2b, 0xb1, 0x49, 0x92, 0xc6, 0x84, 0x61, 0x8c};
	uint8_t ad[] = {};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {0x15, 0xcd, 0x77, 0x73, 0x2f, 0x9d, 0x0c, 0x4c, 0x6e, 0x58, 0x1e, 0xf4, 0x00, 0x87, 0x6a, 0xd9, 0x18, 0x8c, 0x5b, 0x88, 0x50, 0xeb, 0xd3, 0x82, 0x24, 0xda, 0x95, 0xd7, 0xcd, 0xc9, 0x9f, 0x7a, 0xcc};
	uint32_t expected_size = 33;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 33, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 6
int test_case_dec_with_message_with_ad()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x65, 0x49, 0xf9, 0xbf, 0x10, 0xac, 0xba, 0x0a, 0x45, 0x1d, 0xbb, 0x24, 0x84, 0xa6, 0x0d, 0x90};
	uint8_t ciphertext[] = {0x10, 0x9f, 0x8a, 0x16, 0x8b, 0x36, 0xdf, 0xad, 0xe0, 0x26, 0x28, 0xa9, 0xe1, 0x29, 0xd5, 0x25, 0x7f, 0x03, 0xcc, 0x79, 0x12, 0xae, 0xfa, 0x79, 0x72, 0x9b, 0x67, 0xb1, 0x86, 0xa2, 0xb0, 0x8f};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
	uint32_t expected_size = 32;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 32, ad, 16);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 6
int test_case_dec_with_message_with_ad_wrong_tag()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x00, 0x49, 0xf9, 0xbf, 0x10, 0xac, 0xba, 0x0a, 0x45, 0x1d, 0xbb, 0x24, 0x84, 0xa6, 0x0d, 0x90};
	uint8_t ciphertext[] = {0x10, 0x9f, 0x8a, 0x16, 0x8b, 0x36, 0xdf, 0xad, 0xe0, 0x26, 0x28, 0xa9, 0xe1, 0x29, 0xd5, 0x25, 0x7f, 0x03, 0xcc, 0x79, 0x12, 0xae, 0xfa, 0x79, 0x72, 0x9b, 0x67, 0xb1, 0x86, 0xa2, 0xb0, 0x8f};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 0;
	int expected_authentication_failed = 1;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 32, ad, 16);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 7
int test_case_dec_with_message_with_ad2()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0x0b, 0x3f, 0x10, 0xe3, 0x93, 0x3c, 0x78, 0x19, 0x0b, 0x24, 0xb3, 0x30, 0x08, 0xbf, 0x80, 0xe9};
	uint8_t ciphertext[] = {0x7d, 0x77, 0x22, 0x03, 0xfa, 0x38, 0xbe, 0x29, 0x6d, 0x8d, 0x20, 0xd8, 0x05, 0x16, 0x31, 0x30, 0xc6, 0x9a, 0xba, 0x8c, 0xb1, 0x6e, 0xd8, 0x45, 0xc2, 0x29, 0x6c, 0x61, 0xa8, 0xf3, 0x4b, 0x39, 0x4e};
	uint8_t ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {0x42, 0x28, 0x57, 0xfb, 0x16, 0x5a, 0xf0, 0xa3, 0x5c, 0x03, 0x19, 0x9f, 0xb8, 0x95, 0x60, 0x4d, 0xca, 0x9c, 0xea, 0x6d, 0x78, 0x89, 0x54, 0x96, 0x2c, 0x41, 0x9e, 0x0d, 0x5c, 0x22, 0x5c, 0x03, 0x27};
	uint32_t expected_size = 33;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 33, ad, 17);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Test vector 8
int test_case_dec_extreme()
{
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t tag[] = {0xa6, 0x16, 0xf0, 0xc2, 0x18, 0xe1, 0x8b, 0x52, 0x6c, 0xf2, 0xa3, 0xf8, 0xc1, 0x15, 0xe2, 0x62};
	uint8_t ciphertext[] = {,};
	uint8_t ad[] = {};
	uint32_t msg_size;
	int authentication_failed;

	uint8_t expected[] = {};
	uint32_t expected_size = 512;
	int expected_authentication_failed = 0;

	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 512, ad, 513);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	if (msg != NULL) {
		free(msg);
	}

	printf("Result: %s\n", failed ? "Failed" : "Correct");
	return failed;
}

// Performance test
int performance_test_dec_1MB()
{
	// generate test vector
	uint8_t key[32] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
	uint8_t nonce[15] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e};
	uint8_t* message = calloc(1048576, sizeof(uint8_t));
	uint8_t* ad = calloc(1048576, sizeof(uint8_t));
	uint32_t msg_size;
	int authentication_failed;

	uint8_t* ciphertext = Deoxys_AE2_encrypt_buffer(key, nonce, message, 1048576, ad, 1048576);

	clock_t begin = clock();
	uint8_t* msg = Deoxys_AE2_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, &ciphertext[1048576], ciphertext, 1048576, ad, 1048576);
	clock_t end = clock();

	free(ciphertext);
	free(message);
	if (msg != NULL) {
		free(msg);
	}
	free(ad);

	double taken = (double)(end - begin) / CLOCKS_PER_SEC;
	printf("Time taken for decrypting 1MB: %fs\n", taken);
	return taken;
}

int main()
{
	// Encryption tests
/*	printf("Testing Encryption:\n");
	test_case_no_message_no_ad();
	test_case_no_message_with_ad();
	test_case_no_message_with_ad2();
	test_case_with_message_no_ad();
	test_case_with_message_no_ad2();
	test_case_with_message_with_ad();
	test_case_with_message_with_ad2();
	test_case_extreme();
*//*
	printf("\nTesting Decryption:\n");
	test_case_dec_no_message_no_ad();
	test_case_dec_no_message_no_ad_wrong_tag();
	test_case_dec_no_message_with_ad();
	test_case_dec_no_message_with_ad_wrong_tag();
	test_case_dec_no_message_with_ad2();
	test_case_dec_with_message_no_ad();
	test_case_dec_with_message_no_ad2();
	test_case_dec_with_message_with_ad();
	test_case_dec_with_message_with_ad_wrong_tag();
	test_case_dec_with_message_with_ad2();
	test_case_dec_extreme();
*/
	printf("\nEncryption Performance:\n");
	performance_test_1MB();
	//performance_test_1GB();
/*
	printf("\nDecryption Performance:\n");
	performance_test_dec_1MB();

*/	return 0;
}