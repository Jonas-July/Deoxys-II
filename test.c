...
int test_case_dec_no_message_no_ad()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 0);
...
int test_case_dec_no_message_with_ad()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 0, ad, 32);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int test_case_dec_with_message_no_ad()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 32, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int test_case_dec_with_message_no_ad2()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 33, ad, 0);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int test_case_dec_with_message_with_ad()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 32, ad, 16);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int test_case_dec_with_message_with_ad2()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 33, ad, 17);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int test_case_dec_extreme()
{
	...
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, tag, ciphertext, 512, ad, 513);
	int failed = memcmp(msg, expected, expected_size) == 0 ? 0 : 1;
	failed += msg_size == expected_size ? 0 : 2;
	failed += authentication_failed == expected_authentication_failed ? 0 : 4;
	free(msg);
...
int performance_test_dec_1MB()
{
	...
	uint8_t* ciphertext = Deoxys_II_encrypt_buffer(key, nonce, message, 1048576, ad, 1048576);

	clock_t begin = clock();
	uint8_t* msg = Deoxys_II_decrypt_buffer(&authentication_failed, &msg_size, key, nonce, &ciphertext[1048576], ciphertext, 1048576, ad, 1048576);
	clock_t end = clock();
